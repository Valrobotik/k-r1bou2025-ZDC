...
def edgeArucoDetection(img : np.ndarray, detector : cv2.aruco.ArucoDetector = None) :
    ...
    if ids is None :
        rospy.logerr("(CAMERA POLE) No aruco found while getting the edges")
        return None
    ...
    if nb_ar != 4 :
        rospy.logerr("(CAMERA POLE) Not all arucos found while getting the edges")
        return None
    return arPos
...
def unwarp_img(img : np.ndarray, detector : cv2.aruco.ArucoDetector = None, perspective : tuple = None, wimg = False) :
